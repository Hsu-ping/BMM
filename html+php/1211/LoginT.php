<?php
	...
	//sql query
	$result = mysqli_query($con, "SELECT * FROM `teacher` WHERE teacher.UserMail = '$NewID' and teacher.UserPassWord = '$Newpass'");
	$row = mysqli_fetch_array($result, MYSQLI_NUM);
	
	//account and password checking
	if(!$NewID || !$Newpass)
		echo "帳號密碼不得為空";
	else if($result && $row[2] == $NewID && $row[3] == $Newpass)
	{
		echo "$row[1] , 歡迎進入模擬選課系統!";
